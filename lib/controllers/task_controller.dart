...
class TaskController extends ChangeNotifier {
  ...
  void toggleTask(int index) {
    ...
    if (!wasCompleted && task.isCompleted) {
      print('Task "${task.title}" completed with ${task.minutesSpent} minutes spent');
    }
  ...
  void addMinutesToTask(int taskIndex, int minutes) {
    print('addMinutesToTask called - index: $taskIndex, minutes: $minutes, total tasks: ${_tasks.length}');
    if (taskIndex >= 0 && taskIndex < _tasks.length) {
      ...
      print('Added $minutes minutes to task "${_tasks[taskIndex].title}" (${oldMinutes} -> ${_tasks[taskIndex].minutesSpent} minutes)');
      _saveTasks();
      notifyListeners();
    } else {
      print('Invalid task index: $taskIndex (valid range: 0-${_tasks.length - 1})');
    }
  }

  // Get the index of the first uncompleted task (for time tracking)
  int? getFirstUncompletedTaskIndex() {
    print('getFirstUncompletedTaskIndex called - Total tasks: ${_tasks.length}');
    for (int i = 0; i < _tasks.length; i++) {
      print('Task $i: "${_tasks[i].title}" - Completed: ${_tasks[i].isCompleted}');
  ...
  List<Task> get allTasksSorted => List.from(_tasks)
